...
import type { ProjectCalEvent } from "@/types";
...
interface ProjectCalendarPanelProps {
  events: ProjectCalEvent[];
  onEventsChange: (events: ProjectCalEvent[]) => void;
}

export function ProjectCalendarPanel({ events, onEventsChange }: ProjectCalendarPanelProps) {
  ...
  const eventsByDate = useMemo(() => {
    const map: Record<string, ProjectCalEvent[]> = {};
  ...
  const addEvent = () => {
    ...
    onEventsChange([...events, { id: crypto.randomUUID(), date: selected, title, completed: false }]);
    setDraft("");
  };

  const removeEvent  = (id: string) => onEventsChange(events.filter((e) => e.id !== id));
  const toggleEvent  = (id: string) => onEventsChange(events.map((e) => e.id === id ? { ...e, completed: !e.completed } : e));
          ...
          upcoming.map((ev) => (
            <div key={ev.id} className="group flex items-start gap-2.5 px-2.5 py-2 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] transition duration-200 ease-in-out">
              ...
              <button
                ...
                className={cn(
                  "shrink-0 mt-px w-4 h-4 rounded-full border flex items-center justify-center transition duration-200 ease-in-out",
                  ev.completed
                    ? "opacity-100 bg-primary/80 border-primary/80 text-black"
                    : "opacity-0 group-hover:opacity-100 border-white/25 hover:border-primary/60"
                )}
              >
                {ev.completed && <Check className="h-2.5 w-2.5" strokeWidth={3} />}
              </button>

              <span className="shrink-0 mt-px text-[10px] font-medium text-primary/55 whitespace-nowrap">{fmtShort(ev.date)}</span>
              <span className={cn("flex-1 text-[11px] leading-snug break-words min-w-0 transition-colors duration-200", ev.completed ? "text-white/25 line-through" : "text-white/65")}>
                {ev.title}
              </span>
              <button onClick={() => removeEvent(ev.id)} className="opacity-0 group-hover:opacity-100 shrink-0 mt-px text-white/25 hover:text-white/65 transition duration-200 ease-in-out">
                <X className="h-3 w-3" />
              </button>
            </div>
          ))
