...
export function ProjectNotesPanel({ notes, onNotesChange }: ProjectNotesPanelProps) {
  ...
  const removeNote = (id: string) => onNotesChange(notes.filter((n) => n.id !== id));
    ...
    <div className="h-full flex flex-col px-5 pt-4 pb-5 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 space-y-2 mb-3">
        {notes.length === 0 ? (
          ...
          notes.map((note) => (
            <div key={note.id} className="group relative px-3 py-2.5 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] transition duration-200 ease-in-out">
